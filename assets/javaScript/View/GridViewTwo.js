...
cc.Class({
    ...
    setListener: function(){
        this.node.on(cc.Node.EventType.TOUCH_START, function(eventTouch){
            // GameDataMgr.setDataByType(E_GameData_Type.appPower,120)
            // let powernum = GameDataMgr.getDataByType(E_GameData_Type.appPower);
            // if(powernum<=0){
            //     UI_Tip.Instance.showTip("体力不足不能消除");
            //     return;
            // }
    ...
    updateView: function(changeModels){
        ...
        for(var i in changeModels){
            ...
            if(!viewInfo){
                ...
                var aniView = cc.instantiate(this.aniPre[type]);
                aniView.parent = this.node;
                // aniView.getComponent(cc.Sprite).spriteFrame = this.defaultFrame;
                var cellViewScript = aniView.getComponent("CellView");
